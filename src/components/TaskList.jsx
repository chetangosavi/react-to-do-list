    ...
    <div key={tasks.index}>
      <div className="flex justify-between items-center border px-2 py-2 w-[438px]  mt-2 ">
        <div className="text-xl">
          {isTaskComplete ? (
            <FaCheckSquare onClick={handleOnChange} />
          ) : (
            <MdCheckBoxOutlineBlank onClick={handleOnChange} />
          )}
        </div>
        <div>
          {isTrue ? (
            ...
            <div
              className={`${isTaskComplete ? "line-through" : "no-underline"}`}
            >
              {tasks.element}
            </div>
          )}
        ...
        <div className="flex gap-2 text-lg">
          <button className="border p-1">
            {isTrue ? (
              <RxCross2 onClick={() => setIsTrue(!isTrue)} />
            ) : (
              <CiEdit onClick={() => setIsTrue(!isTrue)} />
            )}
          </button>
